...
OUTPUT_DIR <- "/home/vbarbo/project_2021/scripts/paper_1/all_figures_2021.12.28"

# load object mt_jurkat_allMethods_notFiltered
load("/home/vbarbo/project_2021/projects/iso-seq_variant_calling/scripts_for_all_analyses_in_the_paper/3_creating_master_table/jurkat/mt_jurkat_allMethods_notFiltered_v6.RData")
# load object mt_wtc11_allMethods_notFiltered
load("/home/vbarbo/project_2021/projects/iso-seq_variant_calling/scripts_for_all_analyses_in_the_paper/3_creating_master_table/wct11/mt_wtc11_allMethods_notFiltered_v6.RData")

# load object mt_jurkat_allMethods_filtered
load("/home/vbarbo/project_2021/projects/iso-seq_variant_calling/scripts_for_all_analyses_in_the_paper/3_creating_master_table/jurkat/mt_jurkat_allMethods_filtered_v6.RData")
# load object mt_wtc11_allMethods_filtered
load("/home/vbarbo/project_2021/projects/iso-seq_variant_calling/scripts_for_all_analyses_in_the_paper/3_creating_master_table/wct11/mt_wtc11_allMethods_filtered_v6.RData")
```
...
dat <- dat_jurkat %>% 
  { paste(.[,1], .[,2], sep="-") } %>% 
  .[dat_jurkat$in_jurkat_dna_merged==1] %>% 
  data.frame(pos=., Dataset="Jurkat")
dat <- dat_wtc11 %>% 
  { paste(.[,1], .[,2], sep="-") } %>% 
  .[dat_wtc11$in_allen==1] %>% 
...
# jurkat
k <- !( dat_jurkat$variantDensity_jurkatDnaMerged > 3 )
dat_jurkat <- dat_jurkat[k,]
dim(dat_jurkat)
# wtc11
k <- !( dat_wtc11$variantDensity_allen > 3 )
...
k <- select(dat1, in_dv_s_fc, in_c3_mix, in_gatk_s, in_jurkat_dna_merged) %>% 
...
vartype_tbl <- select(dat1, variantType_jurkat_dna_merged, variantType_dv_s_fc,
                      variantType_c3_mix, variantType_gatk_s,
                      variantType_jurkat_dna_merged)
...
# the methods say.
truth_is_na <- vartype_tbl$variantType_jurkat_dna_merged == "NA"
varTypeLevels <- c("deletion", "hetAlt", "homRef", "insertion", "NA", "snp")
typeCounts <- subset(vartype_tbl, select=-variantType_jurkat_dna_merged) %>% 
  apply(1, function(x){
...
k <- vartype_tbl$variantType_jurkat_dna_merged == "NA"
vartype_tbl$variantType_jurkat_dna_merged[k] <- vartype[k]

### don't separate by indel type
vartype_tbl$variantType_jurkat_dna_merged <- recode(
  vartype_tbl$variantType_jurkat_dna_merged,
  "deletion"="indel", "insertion"="indel"
)
table(vartype_tbl$variantType_jurkat_dna_merged)


dat1$varaint_type <- vartype_tbl$variantType_jurkat_dna_merged
dat1 <- filter( dat1, !(varaint_type %in% c("NA", "hetAlt", "homRef")) )
dim(dat1)
dat1 <- select(dat1, in_dv_s_fc, in_c3_mix, in_gatk_s, in_jurkat_dna_merged, varaint_type)
...
dat1 <- rename(dat1, "SNCR+fC+DV"="in_dv_s_fc", "Clair3-mix"="in_c3_mix",
               "SNCR+GATK"="in_gatk_s", "Ground truth"="in_jurkat_dna_merged",
               "Varaint Type"="varaint_type")

# p <- UpSetR:upset(dat1, order.by = "freq")
...
### filter out variant that are not in the ground truth and not called by any method
k <- select(dat1, in_dv_s_fc, in_c3_mix, in_gatk_s, in_allen) %>% 
  apply(1, function(x) all(x==0))
dat1 <- dat1[!k,]
dim(dat1)
...
### decide the type of the variants
vartype_tbl <- select(dat1, variantType_allen, variantType_dv_s_fc,
                      variantType_c3_mix, variantType_gatk_s,
                      variantType_allen)
...
# second criterion: it the ground truth doens't know the variant type, consider what
# the methods say.
truth_is_na <- vartype_tbl$variantType_allen == "NA"
varTypeLevels <- c("deletion", "hetAlt", "homRef", "insertion", "NA", "snp")
typeCounts <- subset(vartype_tbl, select=-variantType_allen) %>% 
  apply(1, function(x){
    x <- factor(x, levels=varTypeLevels)
    x <- table(x)
    x
...
k <- vartype_tbl$variantType_allen == "NA"
vartype_tbl$variantType_allen[k] <- vartype[k]

### don't separate by indel type
vartype_tbl$variantType_allen <- recode(
  vartype_tbl$variantType_allen,
  "deletion"="indel", "insertion"="indel"
)
table(vartype_tbl$variantType_allen)


dat1$varaint_type <- vartype_tbl$variantType_allen
dat1 <- filter( dat1, !(varaint_type %in% c("NA", "hetAlt", "homRef")) )
dim(dat1)
dat1 <- select(dat1, in_dv_s_fc, in_c3_mix, in_gatk_s, in_allen, varaint_type)
dat1$varaint_type <- recode(dat1$varaint_type, "indel"="Indels", "snp"="SNPs")
dat1 <- rename(dat1, "SNCR+fC+DV"="in_dv_s_fc", "Clair3-mix"="in_c3_mix",
               "SNCR+GATK"="in_gatk_s", "Ground truth"="in_allen",
               "Varaint Type"="varaint_type")
...
k <- file.path(OUTPUT_DIR, "fc_algorithm_be.png")
p_1a <- draw_image(k)
p_1a <- ggdraw() + p_1a

# load n-cigar schematic figure (Fig. 1c)
k <- file.path(OUTPUT_DIR, "schematic_n_cigar_reads_explanation_be.png")
p_1c <- draw_image(k)
p_1c <- ggdraw() + p_1c
...
# plots together
p_bc <- plot_grid(p_1b, p_1c,
                  labels=c("B", "C"), nrow=2, rel_heights=c(1,.6),
                  label_size=20)
p_abc <- plot_grid(p_1a, p_bc,
                   labels=c("A", NULL), nrow=1, rel_widths=c(.9, 1),
                   label_size=20)
p_ef <- plot_grid(p_1e, p_1f, 
                  labels=c("E", "F"), nrow=2, rel_heights=c(1,1),
                  label_size=20)
p_def <- plot_grid(p_1d, p_ef,
                   labels=c("D", NULL), nrow=1, rel_widths=c(1, 1),
                   label_size=20)
p_1 <- plot_grid(p_abc, p_def, labels=c(NULL), nrow=2, rel_heights=c(.8, 1),
                 label_size=20)

fp <- file.path(OUTPUT_DIR, "fig_1.png")
...
# these objects were created in the script: /home/vbarbo/project_2021/projects/variantCallingFromIsoSeq/scripts_for_all_analyses_in_the_paper/splice_junction_proximity_analysis/splice_junction_proximity.R
load("~/load_later/near_splice_junctions/data_needed_to_make_the_chart.Rdata")
...
sj_proximity_snps <- lapply(sj_proximity_snps, function(x){
  ...
  x <- rename(x, "Precision measures"="Measures")
  x
})
...
p_2a <- ggplot(sj_proximity_snps$acc_sj,
            aes(x=is_near, y=Score, group=`Precision measures`,
                colour=`Precision measures`)) +
...
# this loads the object: dat_full
# # it was created in the script: /home/vbarbo/project_2021/projects/variantCallingFromIsoSeq/scripts_for_all_analyses_in_the_paper/homopolymer_analysis/identify_indels_within_and_outside_homopolimer_repeats.R
load("~/load_later/homopolymer_analysis/all_methods_info_mtFilteredV5_filterVariantDensityIsoSeq_noNearSJ.RData")

# load("~/load_later/homopolymer_analysis/all_methods_info_mtFilteredV5_filterVariantDensityIsoSeq_noNearSJ_isoSeqCover20.RData")


dat_full <- lapply(dat_full, function(x){
  x$variant_type <- recode(x$variant_type, "deletion"="Deletion",
                           "insertion"="Insertion")
  x$method <- recode(x$method, "Clair3 mix"="Clair3-mix",
                     "SNCR+FC+DeepVariant"="SNCR+fC+DeepVariant")
  x <- rename(x, "Precision measures"="Classification")
  x
})
...
p_2c <- ggplot(dat_full$class_counts,
            aes(x=.data$homopolymer_length_intervals, y=.data$percent,
                group=.data$`Precision measures`, colour=.data$`Precision measures`)) +
  ...
  ylab("Score") +
  theme(text = element_text(size=20)) +
  geom_text( data=dat_full$dat_text, mapping= aes(x=x, y=y, label=label), size=5 ) +
  scale_color_manual(values=method_cols) +
  guides(colour = guide_legend(override.aes = list(size = 1, shape = 11))) +
  theme(legend.position="bottom") +
  NULL
...
p_ab <- plot_grid(p_2a, p_2b, labels=c("D", "E"), nrow=1, rel_widths=c(1,1),
                  label_size=20)
p_cd <- plot_grid(p_2c, p_2d, labels=c("F", "G"), nrow=1, rel_widths=c(1,.3),
                  label_size=20)
p_2 <- plot_grid(p_ab, p_cd, nrow=2, rel_widths=c(1,1), rel_heights=c(.8,1))
...
fp <- file.path(OUTPUT_DIR, "deepvariantPipelines_perCoverrage_indelSnpSepareted.png")
png(fp, width=900)
p_s2
dev.off()
```
...
```{r}
load("/home/vbarbo/load_later/homopolymer_analysis/mt_wtc11_allMethods_filtered_v5_filterVariantDensityIsoSeqAllMethods.RData")
dat_wtc11 <- mt_wtc11_allMethods_filtered_filterVariantDensityIsoSeqAllMethods
...
dat_wtc11 <- filter(dat_wtc11, variantDensity_allen<=1 & 
                      variantDensity_dv_dvS_dvSFc_c3_c3S_c3SFc_c3Mix_gatkS<=1)
dat_wtc11 <- filter(dat_wtc11, is_near_ss==0)

# dat_wtc11 <- filter(dat_wtc11,totalNumReads>=10)

# dat_wtc11 <- filter(dat_wtc11,isoSeq_coverage<30000)

# dat_wtc11 <- mutate(dat_wtc11, log10_isoSeq_coverage=log10(isoSeq_coverage))
...
fp <- file.path(OUTPUT_DIR, "correlation_nCigarReadProportion_coverage.png")
png(fp, width=600)
p_s3
dev.off()
...
### (Fig. S4) compare pipelines that use DeepVariant, per n-cigar read count
...
# METHOD_NAMES <- c("dv", "dv_s", "dv_s_fc", "c3", "c3_s", "c3_s_fc", "gatk_s")
# dat_jurkat <- add_variant_density_of_a_method(dat_jurkat, 201, METHOD_NAMES)
# k <- dat_jurkat$variantDensity_dv_dvS_dvSFc_c3_c3S_c3SFc_gatkS <= 3
# dat_jurkat <- dat_jurkat[k,]
...
dat_jurkat_cover10to20 <- filter(dat_jurkat, isoSeq_coverage==cover[1])

dat_wtc11 <- mt_wtc11_allMethods_filtered
# ##################### if i filter by iso-seq vanraint dentisy (all methods) the result much better.
# METHOD_NAMES <- c("dv", "dv_s", "dv_s_fc", "c3", "c3_s", "c3_s_fc", "gatk_s")
# dat_wtc11 <- add_variant_density_of_a_method(dat_wtc11, 201, METHOD_NAMES)
# k <- dat_wtc11$variantDensity_dv_dvS_dvSFc_c3_c3S_c3SFc_gatkS <= 3
# dat_wtc11 <- dat_wtc11[k,]
# # dat_wtc11 <- filter(dat_wtc11, is_near_ss==0)
# #####################
...
dat_wtc11_cover10to20 <- filter(dat_wtc11, isoSeq_coverage==cover[1])
...
p_s4 <- ggplot(dat, aes(.data$recall, .data$precision, colour=.data$method)) +
  ...
  xlab("Recall") +
  ylab("Precision") +
  labs(colour="Method", size="Proportion of N-cigar reads") +
  # scale_x_continuous(breaks=seq(.5, 1, .1), labels=seq(.5, 1, .1)) +
  scale_color_manual(values=method_cols) +
  guides(colour = guide_legend(order=1)) +
  NULL
...
fp <- file.path(OUTPUT_DIR, "deepvariantPipelines_perNCigarReadCount_indelSnpSepareted_cover10.png")
...
fp <- file.path(OUTPUT_DIR, "clair3Pipelines_perCoverrage_indelSnpSepareted.png")
png(fp, width=900, height=650)
p_s5
dev.off()
```
...
fp <- file.path(OUTPUT_DIR, "sncrNaocaller_perCoverrage_indelSnpSepareted.png")
png(fp, width=900, height=650)
p_s6
dev.off()
```
...
write.table(dat, file="~/keita/best_pipelines_performance_table.txt", row.names=FALSE, quote=FALSE, sep="\t")
```




### tables not included in the paper

#### compare the deepvariant-based pipelines
...
dat <- calculate_precision_recall_for_multi_master_tables(
  ...
  experiment_names = c("Jurkat", "WTC-11"),
  method_names = c("dv", "dv_s", "dv_s_fc"),
  output_method_names = c("DV", "SNCR+DV", "SNCR+fC+DV"),
  data_names = "isoSeq",
  truth_names = c("jurkat_dna_merged", "allen"),
  coverage_thresholds = c(3, 15, 40, 100),
  what = "snps_indels"
...
dat$Dataset <- factor(dat$Dataset, levels=c("Jurkat", "WTC-11"), ordered=TRUE)
dat$Variant_type <- factor(dat$Variant_type, levels=c("Indels", "SNPs"), ordered=TRUE)
dat$Pipeline <- factor(dat$Pipeline, levels=c("DV", "SNCR+DV", "SNCR+fC+DV"), ordered=TRUE)

dat <- arrange(dat, Dataset, Variant_type, Mim_coverage, Pipeline)
(dat_dv <- dat)

write.table(dat, file="~/keita/deepvariant_pipelines_performance_table.txt", row.names=FALSE, quote=FALSE, sep="\t")
```
...
dat <- calculate_precision_recall_for_multi_master_tables(
  ...
  experiment_names = c("Jurkat", "WTC-11"),
  method_names = c("c3", "c3_s", "c3_s_fc"),
  output_method_names = c("Clair3", "SNCR+Clair3", "SNCR+fC+Clair3"),
  data_names = "isoSeq",
  truth_names = c("jurkat_dna_merged", "allen"),
  coverage_thresholds = c(3, 15, 40, 100),
  what = "snps_indels"
...
dat$Dataset <- factor(dat$Dataset, levels=c("Jurkat", "WTC-11"), ordered=TRUE)
dat$Variant_type <- factor(dat$Variant_type, levels=c("Indels", "SNPs"), ordered=TRUE)
dat$Pipeline <- factor(dat$Pipeline, levels=c("Clair3", "SNCR+Clair3", "SNCR+fC+Clair3"), ordered=TRUE)

dat <- arrange(dat, Dataset, Variant_type, Mim_coverage, Pipeline)
(dat_c3 <- dat)

write.table(dat, file="~/keita/clair3_pipelines_performance_table.txt", row.names=FALSE, quote=FALSE, sep="\t")
```
