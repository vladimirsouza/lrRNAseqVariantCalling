...
OUTPUT_DIR <- "/home/vbarbo/project_2021/paper_analysis/secondary_analyses/short_reads/figures_and_results/gatk_deepvariantNotDv"

# load WTC-11 not-filtered master table 
mt_wtc11_allMethods_notFiltered <- readRDS("/home/vbarbo/project_2021/paper_analysis/secondary_analyses/short_reads/extra_files/master_tables/mt_wtc11_gatk_deepvariantButDv_notFiltered_v1.rds")

# load WTC-11 filtered master table 
mt_wtc11_allMethods_filtered <- readRDS("/home/vbarbo/project_2021/paper_analysis/secondary_analyses/short_reads/extra_files/master_tables/mt_wtc11_gatk_deepvariantButDv_filtered_v1.rds")
# mt_wtc11_allMethods_filtered <- readRDS("/home/vbarbo/project_2021/paper_analysis/secondary_analyses/short_reads/extra_files/master_tables/mt_wtc11_gatk_deepvariantButDv_filtered_v2_removeGatkLongDeletions.rds")
# mt_wtc11_allMethods_filtered <- readRDS("/home/vbarbo/project_2021/paper_analysis/secondary_analyses/short_reads/extra_files/master_tables/mt_wtc11_gatk_deepvariantButDv_filtered_v2_removeGatkLongDeletionsInsertions.rds")
...
# extra figure (not in the paper)

get read coverage distribution of site where the gound truth shows a variant. compare iso-seq, nanopore, and illumina.

```{r}
dat_isoseq <- readRDS("/home/vbarbo/project_2021/paper_analysis/extra_files/master_tables/mt_wtc11_allMethods_filtered_v7.rds")
dat_nanopore <- readRDS("~/project_2021/paper_analysis/secondary_analyses/nanopore/extra_files/ENCFF961HLO/clair3_gatk_deepvariant/master_tables/mt_wtc11_clair_gatk_dv_filtered_v1.rds")
dat_illumina <- readRDS("~/project_2021/paper_analysis/secondary_analyses/short_reads/extra_files/master_tables/mt_wtc11_gatk_deepvariantButDv_filtered_v1.rds")

isoseq_cover <- dat_isoseq$isoSeq_coverage[dat_isoseq$in_allen==1]
nanopore_cover <- dat_nanopore$Nanopore_coverage[dat_nanopore$in_allen==1]
illumina_cover <- dat_illumina$illumina_coverage[dat_illumina$in_allen==1]

k_max <- max( c(isoseq_cover, nanopore_cover, illumina_cover) )
k_levels <- 0:k_max
isoseq_cover <- factor(isoseq_cover, levels=k_levels)
nanopore_cover <- factor(nanopore_cover, levels=k_levels)
illumina_cover <- factor(illumina_cover, levels=k_levels)

isoseq_cover <- table(isoseq_cover)
nanopore_cover <- table(nanopore_cover)
illumina_cover <- table(illumina_cover)

k <- c(isoseq_cover, nanopore_cover, illumina_cover)
k1 <- rep(
  c("isoseq", "nanopore", "illumina"),
  c( length(isoseq_cover), length(nanopore_cover), length(illumina_cover) )
)
dat_cover_all <- data.frame(
  counts = as.numeric(k),
  coverage = factor( names(k), levels=k_levels, ordered=TRUE ),
  sequencing = k1
)

dat_cover_all2 <- filter(dat_cover_all, as.numeric(as.vector(coverage)) <= 40)
dat_cover_all2 <- filter(dat_cover_all2, as.numeric(as.vector(coverage)) >= 3)

p <- ggplot(dat_cover_all2, aes(x=coverage, y=counts, fill=sequencing)) +
  geom_bar(stat="identity", position=position_dodge())

fp <- file.path(OUTPUT_DIR, "histogram_coverage_sequencing.png")
png(fp,  width=1420, height=600)
p
dev.off()
```
...
Compare methods per ranges of read coverage.

```{r}
experiment_names <- c("WTC-11")
method_names <- c("dvWes_sncr", "dvWes_sncr_fc", "gatk")
output_method_names <- c("SNCR+DeepVariantWES", "SNCR+fC+DeepVariantWES", "GATK")
coverage_ranges <- data.frame( s=c(3, 10, 30,  80, 200, 400, 1000),
                               e=c(5, 15, 40, 100, 250, 500, 1500) )
...
ndat
#   Dataset   Type  3--5 10--15 30--40 80--100 200--250 400--500 1000--1500
# 1  WTC-11 Indels 11848   2842    842     412      280      160         97
# 2  WTC-11   SNPs 62498  15840   5158    2538     1772     1130        779


method_cols <- hue_pal()(9)[c(2,1,7)]
...
p_1d <- ggplot(dat, aes(.data$recall, .data$precision, colour=.data$method)) +
  ...
  xlab("Recall") +
  ylab("Precision") +
  labs(colour="Method", size="Illunina short read coverage ranges") +
...
# fp <- file.path(OUTPUT_DIR, "bestPipelinesAndNcs_perRangeCoverrage_indelSnpSepareted_noGatkLongDeletions.png")
# fp <- file.path(OUTPUT_DIR, "bestPipelinesAndNcs_perRangeCoverrage_indelSnpSepareted_noGatkLongDeletionsInsertions.png")
...
Upset plot to compare the best methods

```{r}
method_names <- c("dvWes_sncr", "dvWes_sncr_fc", "gatk")
output_method_names <- c("SNCR+DeepVariantWES", "SNCR+fC+DeepVariantWES",
                         "GATK")
...
# # if all methods agree with one variant type, take it
# j <- number_of_methods
...
# vartype[k1] <- k[k1]
# 
# # if the majority (3) of the methods say it's a variant type, it's okay to take it
# j <- (number_of_methods / 2) + 0.01
# k <- apply(typeCounts, 2, function(x){
#   if(any(x>=j)){
#     varTypeLevels [x>=j] [1]
#   }else{
...
# j <- number_of_methods -1
# na_indx <- varTypeLevels == "NA"
# k <- apply(typeCounts, 2, function(x){
#   if( x[na_indx]==j & any(x==1) ){
#     varTypeLevels[x==1]
#   }else{
#     NA
#   }
...
# fp <- file.path(OUTPUT_DIR, "upsetPlot_bestMethods_wtc11_noGatkLongDeletions.png")
# fp <- file.path(OUTPUT_DIR, "upsetPlot_bestMethods_wtc11_noGatkLongDeletionsInsertions.png")
png(fp, width=800)
p_1f
dev.off()
```
...
# plots together
p_ab <- plot_grid(p_1d, NULL, p_1b,
                  labels=c("A", "", "B"), ncol=1, rel_heights=c(1, .05, 1),
                  label_size=28)

fp <- file.path(OUTPUT_DIR, "fig_illumina.png")
